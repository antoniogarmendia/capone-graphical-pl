				...
				for (EAnnotation an : modifiers) {
					if (an.getDetails().containsKey(Modifiers.MAX_MODIFIER)) {
						...
						if ("1".equals(max_val)) {
							...
							if (fs.isSat()) {
								list.add(new ValidationIssue(	"Multivalued feature '"+sf.getName()+"' receives a collection operation in invariant '"+
				...
				for (EAnnotation an : modifiers) {
					if (an.getDetails().containsKey(Modifiers.MAX_MODIFIER)) {
						...
						if (!"1".equals(max_val)) {
							...
							if (fs.isSat()) 
								list.add(new ValidationIssue(	"Monovalued feature '"+sf.getName()+"' receives a monovalued operation in invariant '"+

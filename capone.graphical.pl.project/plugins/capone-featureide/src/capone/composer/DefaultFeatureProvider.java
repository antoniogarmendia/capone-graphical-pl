...
public class DefaultFeatureProvider implements IFeatureProvider {
	...
	private void createFeatureModel() {
		...
		final IPersistentFormat<IFeatureModel> format = FMFormatManager.getInstance().getFormatByContent(contents, this.featureModelFile.getLocation().toOSString());
		try {
			IPath ip = this.featureModelFile.getLocation();
			Path p = ip.toFile().toPath();
			IFeatureModelFactory fmf = FMFactoryManager.getInstance().getFactory(p, format);
			this.fm = fmf.create();
		} catch (final NoSuchExtensionException e) {			
			try {
				fm = FMFactoryManager.getInstance().
						getFactory(DefaultFeatureModelFactory.ID).create();
